...
int GTD_map::mapCreator()
{
    //select Hard to traverse cells
    randomSelectHard(map, 120, 160, 8);
    ...
    for (int i = 0; i < 4; )
    {
        int ret = randomSelectHighway(map, 120, 160, i + 1);
        if (ret == 0)
        {
            i++;
            signal++;
    ...
    for (int i = 0; i < 120; i++)
    {
        for (int j = 0; j < 160; j++)
        {
            if (map[i][j].isHighway == 'a')
                map[i][j].weight = map[i][j].isHighway;
            else if (map[i][j].isHighway == 'b')
                map[i][j].weight = map[i][j].isHighway;
        }
    }

    //select blocked cells
    randomSelectBlock(map, 120, 160);
    randomSelectStartAndGoal();

    return 0;
}

int GTD_map::randomSelectHighway(node (*map)[160], int row, int column, int index)
{
    ...
    while (r >= 0 && r < row && c >= 0 && c < column)
    {
        if (vOrh == 1)
        {
            for (int j = 0; j < 20 && r < row && r >= 0; j++)
            {
                ...
                if (map[r][c].isHighway == 'n' && isVisited.find((&map[r][c])) == isVisited.end())
                {
                    isVisited[(&map[r][c])] = 1;
                    vect.push_back(&map[r][c]);
                    if (direction == 1)
                        r++;
                    else
                        r--;
        ...
        else
        {
            ...
            {
                if (map[r][c].isHighway == 'n' && isVisited.find((&map[r][c])) == isVisited.end())   //std::find(vect.begin(), vect.end(), &map[r][c]) == vect.end()
                {
                    isVisited[(&map[r][c])] = 1;
                    vect.push_back(&map[r][c]);
                    if (direction == 1)
...
void GTD_map::randomSelectHard(node (*map)[160], int row, int column, int amount)
{
    srand(unsigned(time(NULL)));
    ...
    for (int i = 0; i < amount; i++)
    {
        ...
        for (int j = r - 15; j < r + 15; j++)
        {
            for (int k = c - 15; k < c + 15; k++)
            {
                ...
                if (mark >= 0.5)
                {
                    map[j][k].weight = '2'; // when the random number is 1, we change the weight of the cell to 2
                }
            }
        }
    }
...
void GTD_map::randomSelectBlock(node (*map)[160], int row, int column)
{
    srand(unsigned(time(NULL)));
    ...
    for (int i = 0; i < amount;)
    {
        ...
        if (map[r][c].isHighway != 'a' && map[r][c].isHighway != 'b')
        {
            map[r][c].weight = '0';
            i++;
        }
...
std::vector <std::string> GTD_map::split(std::string s, std::string delim)
{
...
void GTD_map::mapWriter(std::string path)
{
    ...
    for (int i = 0 ; i < 10 && it_start != start.end() && it_goal != goal.end(); i++ , it_start++, it_goal++)
    {
        ...
        for (int i = 0; i < 120; i++)
        {
            ...
            {
                file << map[i][j].weight;
                if (map[i][j].weight == 'a' || map[i][j].weight == 'b')
                    file << map[i][j].index;
                if (j < 159)
                    file << ',';
                counter++;
...
int GTD_map::mapLoader(std::string path, QGraphicsScene *scene)
{
...
int GTD_map::mapAndPathLoader(std::string mapDirect, std::string pathDirect, QGraphicsScene *scene)
{
    ...
    std::string str;
    std::getline(file, str);
    int init_coord_x = std::stoi(str.substr(0,str.find(',')-1));
    int init_coord_y = std::stoi(str.substr(str.find(',')+1));

    QGraphicsRectItem * rect_init = new QGraphicsRectItem();

    rect_init->setRect((init_coord_y) * 10, (init_coord_x) * 10, 10, 10);
    rect_init->setBrush(Qt::red);
    scene->addItem(rect_init);
    ...
    if (recv != "")
    {
        ...
        {
           std::string temp = *it;
           coordinate_x = std::stoi(temp.substr(0, (recv.find(','))));
           coordinate_y = std::stoi(recv.substr(recv.find(',') + 1));
...
void GTD_map::randomSelectStartAndGoal()
{
    srand(unsigned(time(NULL)));
    double rnd_0 ;
    double rnd_1 ;
